            ...
            if (guna2TextBox1.Text != "" && guna2TextBox2.Text != "" && guna2TextBox3.Text != "")
            {
                ...
                if (btn_editProfile.Text == "Сменить пароль")
                {
                    if (guna2TextBox2.Text == guna2TextBox3.Text)
                    {
                        ...
                        string selectP = "SELECT `password` FROM `student` WHERE `password` = @p AND `id` = @id";
                        ...
                        {
                            db.openConnection();
                            string editP = "UPDATE `student` SET `password` = @p WHERE `id` = @id";
                ...
                else if (btn_editProfile.Text == "Сменить логин")
                {
                    ...
                    string selectL = "SELECT `login` FROM `student` WHERE `login` = @l AND `id` = @id";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        string selectP1 = "SELECT `login` FROM `student` WHERE `login` = @l";
                        ...
                        if (dt.Rows.Count > 1)
                        {
                            ...
                            {
                                db.openConnection();
                                string editP = "UPDATE `student` SET `login` = @l WHERE `id` = @id";
