...
namespace CMS.Student
{
    public partial class studentProfile : UserControl
    {
        ...
        {
            label19.Text = "Студент";
        }

        private void guna2Button2_Click(object sender, EventArgs e)
        {
            Editprofile aEP = new Editprofile();
            aEP.label5.Text = label24.Text;
        ...
        private void SaveImageToDatabase(string filePath)
        {
            ...
            string id = label24.Text;

            try
            {
                db.openConnection();
                string query = "UPDATE `student` SET `img` = @ImageData WHERE `id` = @id";
